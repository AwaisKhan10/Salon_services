...
// /// Create a [AndroidNotificationChannel] for heads up notifications
  ...
  showLocalNotificaiton(String number) async {
    const AndroidNotificationDetails androidPlatformChannelSpecifics =
        AndroidNotificationDetails('ST001', 'Android', priority: Priority.high);
    const NotificationDetails platformChannelSpecifics =
        NotificationDetails(android: androidPlatformChannelSpecifics);
    await flutterLocalNotificationsPlugin.show(
      1,
      'Verification OTP',
      'Please enter the otp for phone no verification $number',
  ...
  ///
  /// show local notificaiton
  showBookingNotificaton(String stylistName, String title) async {
    const AndroidNotificationDetails androidPlatformChannelSpecifics =
        AndroidNotificationDetails('ST001', 'Android', priority: Priority.high);
    const NotificationDetails platformChannelSpecifics =
        NotificationDetails(android: androidPlatformChannelSpecifics);
    await flutterLocalNotificationsPlugin.show(
      1,
      'Booking $title',
      'Your appointment ${title.toLowerCase()} with $stylistName',
      platformChannelSpecifics,
    );
  }
...
//now finally get the token from
    ...
    // _moveToTheRespectiveScreen(NotificationModel notificationModel) {
    // print(
    // "=============================> NOTIFICATION/ORDERS/REDEEMS====> ${notificationModel.title}\n${notificationModel.body}");
    //   if (notificationModel.type == NotificationType.order) {
    //     print("ORDERss");
    //     Get.dialog(MyCCdialog(notificationModel.body),
    //         barrierDismissible: false);
    //   } else if (notificationModel.type == NotificationType.redeem) {
    //     print("REDEEM");
    //     Get.dialog(RedeemSuccessfullDialog(), barrierDismissible: false);
    //   } else {
    //     print("OTHER");
    //   }
    // }
    ...
    _handleNotification(RemoteMessage message, bool isOnMessage) {
      ...
      print("NOTIFICATION JSON MODEL +========> ${notificationModel.type}");
      if (isOnMessage) {
        // Get.snackbar(
        ...
        //   onTap: (data) {
        //     // _moveToTheRespectiveScreen(notificationModel);
        //   },
        // );
        // _moveToTheRespectiveScreen(notificationModel);
      } else {
    ...
    FirebaseMessaging.onMessageOpenedApp.listen((RemoteMessage message) {
      ...
      //   print("A NOTIFICATION have arrived===============><=================");
      // }
    });
...
class NotificationModel {
  ...
  NotificationModel(this.title, this.body, this.type);

  NotificationModel.fromJson(json) {
    this.title = json[('title')];
    this.body = json['body'];
    this.type = _getTypeFromString(json['type']);
  }

  _getTypeFromString(String type) {
    switch (type) {
      case 'order':
